...
contract Token is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    /**
     *  @dev Sets the reward distributor contract address.
     *  Only the owner can call this function.
     */
    function setRewardDistributor(address _distributor) public override onlyOwner {
        _rewardDistributor = _distributor;
    }

    /**
     *  @dev Returns the reward distributor contract address.
     */
    function rewardDistributor() public view override returns (address) {
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal virtual {
        if (_rewardDistributor != address(0)) {
            IRewardDistributor(_rewardDistributor).settleOnTransfer(_from, _to);
        }
    }
}
